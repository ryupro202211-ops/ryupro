...
md.append("3. `--week N --body <file>` で `blog/posts/` に記事を生成し、`blog/data/posts.json` の先頭に追加")
md.append("4. `git commit && git push` → GitHub Actions が自動デプロイ\n")
md.append("## ツールの構成\n")
...
md.append("- `tools/render-post.js` … テンプレートへの流し込み。new-post.js と server.js の両方がこれを使う")
md.append("- `tools/new-post.js` … 週次投稿。記事生成／posts.json／sitemap.xml をまとめて更新")
md.append("- `tools/rebuild-posts.js` … posts.json の content から全記事を再生成（サーバー起動不要）")
...
md.append("## 重要な注意: server.js は起動時に記事HTMLを再生成する\n")
md.append("`node server.js`（ローカル開発サーバー／管理画面）は、起動のたびに `blog/data/posts.json` の")
md.append("`content` フィールドから `blog/posts/*.html` を作り直します。")
md.append("そのため **`content` を持たない記事は、サーバーを起動しただけで本文が消えます**。\n")
md.append("- `tools/new-post.js` は必ず `content` を posts.json に書き込みます（対策済み）")
md.append("- `server.js` にも「`content` が空の記事はスキップする」ガードを入れてあります（対策済み）")
md.append("- HTMLを直接手で編集した場合は、同じ内容を posts.json の `content` にも反映すること\n")
md.append("`blog/posts/2026-05-16-gyoza.html` はヒーロー画像を含む手書き記事のため posts.json に `content` を持たない例外で、上記ガードで保護されています。\n")
